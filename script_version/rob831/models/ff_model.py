...
class FFModel(nn.Module, BaseModel):
    ...
    def __init__(self, ac_dim, ob_dim, n_layers, size, learning_rate=0.001):
        ...
        # Define optimizer, which include the network
    ...
    def forward(
        ...
        # Q: Why do we need to concatenate the input?
        # Because we need to input those informations together into delta_network.
        # 
        # print ("Observation from forward in ff_model", obs_normalized.shape) 
        ...
        # Q: What is the meaning of this normalization Step?
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        # print("obs from get_prediction in ff_model", obs.shape)
        new_data_statistics = {
        ...
        prediction, _ = self(
            obs,
            acs,
            new_data_statistics['obs_mean'],
            new_data_statistics['obs_std'],
            new_data_statistics['acs_mean'],
            new_data_statistics['acs_std'],
            new_data_statistics['delta_mean'],
            new_data_statistics['delta_std'],
        )# TODO(Q1) get numpy array of the predicted next-states (s_t+1)
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
        prediction = ptu.to_numpy(prediction)
        # print("reshaped obs from get_prediction in ff_model", prediction.shape)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        n_obs = ptu.from_numpy(next_observations)
        ...
        target = normalize(n_obs - obs, new_data_statistics['delta_mean'], new_data_statistics['delta_std']) # TODO(Q1) compute the normalized target for the model.
        ...
        loss = self.loss(target, delta)# TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
