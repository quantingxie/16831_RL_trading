...
#         return df
  ...
  def _next_observation(self):
        ...
        frame1 = np.zeros((5, LOOKBACK_WINDOW_SIZE + 1))

        # Get the stock data points for the last 5 days and scale to between 0-1

        # print (self.df)
        # print("only one ", self.df.loc[self.current_step: self.current_step + LOOKBACK_WINDOW_SIZE, 'Open'].values/ MAX_SHARE_PRICE)
        ...
        #     self.df.loc[self.current_step: self.current_step + LOOKBACK_WINDOW_SIZE, 'Volume'].values / MAX_NUM_SHARES,
        # ])
        # print(np.shape(self.df.loc[self.current_step: self.current_step + LOOKBACK_WINDOW_SIZE, 'Open'].values))
        ...
        np.put(frame1, range(205), frame)

        # print (frame1)
        
        stacked_frame = frame1.flatten()
        # print("stacked frame shape ==", stacked_frame.shape)

        obs = np.append(stacked_frame, [self.balance / MAX_ACCOUNT_BALANCE,
        self.max_net_worth / MAX_ACCOUNT_BALANCE, 
        self.shares_held / MAX_NUM_SHARES,
        self.cost_basis / MAX_SHARE_PRICE,
        self.total_sales_value / (MAX_NUM_SHARES * MAX_SHARE_PRICE)
        ...
        # print("balance", self.balance/ MAX_ACCOUNT_BALANCE)
        # stacked_obs = obs.flatten()
        # print("stacked obs shape ==", obs.shape)
        return obs
...
# each agent and initialize its open positions to an empty list.
  def reset(self):
        ...
        # assert n_obs.shape == (5 * LOOKBACK_WINDOW_SIZE + 10,), 'observation form reset() is wrong in shape'
...
# Create a calculate_reward function that need to take observations and actions as input
  ...
  def get_reward(self, observations, actions):
    ...
    # print("observations from get reward", observations.shape)

    balance = observations[:,observations.shape[1]-5]
    net_worth = observations[:, observations.shape[1]-4]
    ...
    #done
    # print(self.current_step >= len(self.df.loc[:, 'Open'].values))
    # dones = net_worth <= 0 or self.current_step >= len(self.df.loc[:, 'Open'].values)

    dones = np.logical_or(net_worth <= 0, self.current_step >= len(self.df.loc[:, 'Open'].values))
    # print(dones)
