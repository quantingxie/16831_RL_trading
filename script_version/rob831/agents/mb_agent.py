...
class MBAgent(BaseAgent):
    def __init__(self, env, agent_params):
        ...
        for i in range(self.ensemble_size):
            # Initialize FFmodels by giving the settings needed in __init__ function of FFModel
            model = FFModel(
            ...
            # We could append class instances using a list
            self.dyn_models.append(model)

        # In Model Based RL, we use MPC as our control policy to predict actions based on states. 
        self.actor = MPCPolicy(
            # Why need env here is because MPC need to evaluate the actions by interacting with real environment. 
            self.env,
        ...
        # Initialize replay buffer contains dataset D
        self.replay_buffer = ReplayBuffer()

    def train(self, ob_no: object, ac_na, re_n, next_ob_no, terminal_n):
        ...
        for i in range(self.ensemble_size):
            ...
            # Breaking the data to ensemble size 
            ...
            # use breaked data to update individual dyn_models
            model = self.dyn_models[i] # TODO(Q1)
            # print("actions shape from mb_agnet", actions.shape)
            # print("observation shape from mb_agent ", observations.shape)
    ...
    def add_to_replay_buffer(self, paths, add_sl_noise=False):
        ...
        # Why do we want to add noise? 
        # Add rollout means add individual (s, a, s') pairs?
