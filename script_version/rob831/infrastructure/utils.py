...
def sample_trajectory(env, policy, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    obs = env.reset()
    # print("observation from env reset in utils", obs.shape)
    ...
    while True:
        ...
        # print("observation from step in utils", nobs.shape)
...
def convert_listofrollouts(paths):
    ...
    # print("observation from convert_listofrollouts, utils", observation.shape)

    observations = np.concatenate([path["observation"] for path in paths])
    # print("observations from convert_listofrollouts, utils", observations.shape)
    # observations = observations.reshape(20100, 36)
    actions = np.concatenate([path["action"] for path in paths])
    next_observations = np.concatenate([path["next_observation"] for path in paths])
    # print("next_observations from convert_listofrollouts, utils", next_observations.shape)
    # next_observations = next_observations.reshape(20100, 36)
